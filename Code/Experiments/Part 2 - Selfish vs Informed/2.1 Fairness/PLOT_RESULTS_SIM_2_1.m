% FILE DESCRIPTION:
% Script to plot the results of experiment 2-1 (Fairness Issues in WLANs)
% This script plots article's figures 9b and 9c in Section 5.1.1
...
disp('----------------------------------------------')
disp('PLOT RESULTS OF EXPERIMENT 2-1')
disp('----------------------------------------------')
...
% Load results from experiments in 2.1
load('results_experiment_2_1_1.mat');
load('results_experiment_2_1_2.mat');
load('results_experiment_2_1_3.mat');
load('results_experiment_2_1_4.mat');

% Determine the number of WLANs and iterations
num_wlans = size(tpt_evolution_2_1_1, 2);
total_iterations = size(tpt_evolution_2_1_1, 1);

% Average tpt experienced per WLAN
mean_tpt_per_wlan_selfish = mean(tpt_evolution_2_1_1(1:total_iterations,:), 1);
mean_tpt_per_wlan_shared = mean(tpt_evolution_2_1_2(1:total_iterations,:), 1);
mean_tpt_per_wlan_selfish_egreedy = mean(tpt_evolution_2_1_3(1:total_iterations,:), 1);
mean_tpt_per_wlan_shared_egreedy = mean(tpt_evolution_2_1_4(1:total_iterations,:), 1);
std_per_wlan_selfish = std(tpt_evolution_2_1_1(1:total_iterations,:), 1);
std_per_wlan_shared = std(tpt_evolution_2_1_2(1:total_iterations,:), 1);
std_per_wlan_selfish_egreedy = std(tpt_evolution_2_1_3(1:total_iterations,:), 1);
std_per_wlan_shared_egreedy = std(tpt_evolution_2_1_4(1:total_iterations,:), 1);

% Upper bounds
selfish_upper_bound = [113.2326 82.6295];
shared_upper_bound = [102.3659 82.6295];
max_min = 82.6295;
...
% Save Figure
figName = 'experiment_2_1_mean_tpt';
...
axis([1 20 30 70]);
subplot(2,1,1)
tpt_per_iteration_thompson = mean(tpt_evolution_2_1_1(1 : total_iterations, 1), 2);
...
set(gca,'FontSize', 22)
subplot(2,1,2)
tpt_per_iteration_egreedy = mean(tpt_evolution_2_1_3(1 : total_iterations, 1), 2);
...
set(gca,'FontSize', 22)
% Save Figure
figName = 'experiment_2_1_variability';
savefig(['./Output/' figName '.fig'])
saveas(gcf,['./Output/' figName],'png')